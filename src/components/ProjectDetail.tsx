...
import { ArrowLeft, ExternalLink, Mail, CheckCircle2, Target, Lightbulb, Sparkles, Layout, Zap, ArrowRight, TrendingUp } from "lucide-react";
        ...
        <div className="container mx-auto max-w-5xl px-6 text-center">
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href={project.url} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto">
              <Button size="lg" className="rounded-full bg-primary px-10 h-14 text-base shadow-xl shadow-primary/20 transition-all hover:scale-[1.02] w-full">
                View Live Project
                <ExternalLink className="ml-2 h-4 w-4" />
              </Button>
            </a>
            <Link to="/contact" className="w-full sm:w-auto">
              <Button size="lg" variant="outline" className="rounded-full border-border/80 px-10 h-14 text-base backdrop-blur-md transition-all hover:bg-muted/50 w-full">
                Start a Project
              </Button>
            </Link>
          </div>
        ...
        <div className="container mx-auto max-w-6xl px-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {summaryCards.map((card, i) => (
              <div key={i} className="flex items-start gap-4 p-4 rounded-2xl bg-background/50 border border-border/50 shadow-sm">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/5 text-primary">
                  <card.icon className="h-5 w-5" />
                </div>
                <div>
                  <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">{card.label}</span>
                  <p className="mt-1 text-sm font-semibold text-foreground leading-snug">{card.value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="container mx-auto max-w-5xl px-6 space-y-16 md:space-y-24">
          ...
          <div className="grid grid-cols-1 items-center gap-12 md:grid-cols-2">
            <div className="space-y-6">
              ...
              <h3 className="text-3xl font-bold tracking-tight text-foreground md:text-4xl">Identifying the bottleneck.</h3>
            ...
            <div className="relative overflow-hidden rounded-3xl border border-border/40 bg-card shadow-lg">
              <img src={project.image} alt="Problem Visualization" className="w-full object-cover" />
            </div>
          </div>

          <div className="grid grid-cols-1 items-center gap-12 md:grid-cols-2">
            <div className="order-2 md:order-1 relative overflow-hidden rounded-3xl border border-border/40 bg-card shadow-lg">
              <img src={project.screenshots[0]} alt="Solution Visualization" className="w-full object-cover" />
            </div>
            <div className="order-1 md:order-2 space-y-6">
              ...
              <h3 className="text-3xl font-bold tracking-tight text-foreground md:text-4xl">A modern, scalable fix.</h3>
              <p className="text-lg leading-relaxed text-muted-foreground">
                {project.detailedSolution}
              </p>
              <div className="grid grid-cols-1 gap-3 pt-2">
                {project.features.slice(0, 3).map((feature, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-background/80 border border-border/40">
        ...
        <div className="container mx-auto max-w-4xl px-6">
          <div className="text-center space-y-10 rounded-[3rem] border border-border/60 bg-primary/5 p-12 md:p-20 relative overflow-hidden">
             ...
             <div className="relative space-y-4">
                <h3 className="text-2xl font-bold tracking-tight md:text-4xl">Measurable results, not just a site.</h3>
                <p className="text-muted-foreground max-w-lg mx-auto">{project.impact}</p>
             </div>
             <div className="relative pt-6">
                <div className="text-7xl md:text-9xl font-bold tracking-tighter text-primary">{project.metric}</div>
                <div className="mt-2 text-xs font-bold uppercase tracking-[0.3em] text-muted-foreground">{project.metricLabel}</div>
             </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-20 border-t border-border/40 bg-background">
        ...
        <div className="container mx-auto max-w-3xl px-6 text-center space-y-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-2">
            <Link to="/contact">
              <Button size="lg" className="rounded-full px-12 h-16 text-lg shadow-xl shadow-primary/20 transition-all hover:scale-105">
                Start a Project
              </Button>
            </Link>
            <a href="mailto:hello@gentech.com" className="flex items-center gap-2 font-bold text-foreground hover:text-primary transition-colors text-lg px-8">
